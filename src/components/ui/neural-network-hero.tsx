...
function ShaderBackground() {
  ...
  return (
    <div ref={canvasRef} className="bg-black w-full h-full" aria-hidden>
      <Canvas
        camera={camera}
  ...
  return (
    <section ref={sectionRef} className="relative w-full">
      ...
      <div className="bg-white w-full">
        <div className="mx-auto flex max-w-7xl flex-col items-start gap-6 px-6 py-16 sm:gap-8 sm:py-20 md:px-10 lg:px-16">
          <div ref={badgeRef} className="inline-flex items-center gap-2">
            <span className="text-[11px] font-medium uppercase tracking-wide text-orange-500">{badgeLabel}</span>
            <span className="h-1 w-1 rounded-full bg-gray-400" />
            <span className="text-sm font-normal tracking-tight text-gray-600">{badgeText}</span>
          </div>

          <h1 ref={headerRef} className="max-w-4xl text-left text-4xl font-bold leading-tight tracking-tight text-gray-900 sm:text-5xl md:text-6xl lg:text-7xl">
            {title}
          </h1>

          <p ref={paraRef} className="max-w-3xl text-left text-base font-normal leading-relaxed tracking-normal text-gray-600 sm:text-lg">
            {description}
          </p>
          ...
          <div ref={ctaRef} className="flex flex-wrap items-center gap-4 pt-4">
            {ctaButtons.map((button, index) => (
              <a
                key={index}
                href={button.href}
                className={`rounded-full px-6 py-3 text-sm font-medium tracking-tight transition-all focus:outline-none focus:ring-2 focus:ring-orange-500/30 duration-300 ${
                  button.primary
          ...
          <ul ref={microRef} className="mt-8 flex flex-wrap gap-8 lg:gap-12">
            {microDetails.map((detail, index) => {
              const refMap = [microItem1Ref, microItem2Ref, microItem3Ref];
              const [value, label] = detail.split(' ');
              return (
                <li key={index} ref={refMap[index]} className="flex flex-col gap-1">
